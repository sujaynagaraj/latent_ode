...
parser.add_argument('--preempt-path', default='', help="preemption fix")
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.preempt_path):
        os.makedirs(args.preempt_path)

    start_itr = 0
    try:
        chkpt = torch.load(os.path.join(args.preempt_path, 'latest.pt')) 
        model.load_state_dict(chkpt['model'])
        optimizer.load_state_dict(chkpt['optimizer'])
        start_itr = chkpt['itr']

        torch.set_rng_state(chkpt['torch_random_seed_state'])
        np.random.set_state(chkpt['numpy_random_seed_state'])

        print("Loaded checkpoint with success.")
    except FileNotFoundError:
        print("No checkpoint found.") 

    for itr in range(start_itr, num_batches * (args.niters + 1)):
        optimizer.zero_grad()
        ...
        if itr % (n_iters_to_viz * num_batches) == 0:
            ...
            torch.save({'model':model.state_dict(),
                    ...
                    os.path.join(args.preempt_path, 'latest.pt'))
